    ...
    const mappedLinks = filteredLinks.map((el, index) => {
        return (
            <Link
                href={el.href}
                key={index}
                className="relative text-white transition-colors duration-300 after:content-[''] after:absolute after:w-0 after:h-[2px] after:bg-white after:left-0 after:bottom-[-5px] after:transition-all after:duration-300 hover:after:w-full flex items-center gap-4 text-2xl"
            >
               {el.name}
            </Link>
        );
